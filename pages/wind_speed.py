"""Wind Speed page for 10m and pressure level wind speed visualization."""
...
# Register page
dash.register_page(__name__, path="/wind-speed", name="Wind Speed")

# Page-specific parameters - surface and pressure levels
PAGE_PARAMS = [
    "windspd10m",      # 10m surface wind
    "windspd700hPa",   # 700 hPa
    "windspd800hPa",   # 800 hPa
    "windspd850hPa",   # 850 hPa
    "windspd900hPa",   # 900 hPa
    "windspd950hPa",   # 950 hPa
]
PAGE_ID = "windspd"
...
def update_parameter_options(step, cycle, realm):
    ...
    return create_dropdown_options(options, "windspd10m")
...
def update_field_options(output, validtime, parameter, step, cycle, realm):
    ...
    # Probability outputs - use predefined PoW fields from config
    if output in PROBABILITY_OUTPUTS:
        pow_fields = config.get_probability_fields(parameter)
        if pow_fields:
            options = [{"label": k, "value": str(v)} for k, v in pow_fields.items()]
            # Default to middle threshold
            default_value = str(list(pow_fields.values())[len(pow_fields)//2])
            return visible_style, options, default_value
        return hidden_style, [], None

    # Percentile outputs - use predefined fields from config
...
def load_and_process_data(realm, cycle, step, parameter, output, validtime, field_value, use_native_proj=False):
    ...
    field_label = ""
    colorscale_type = "Wind"
    proj4str = loader.proj4str

    if data_type == "threshold" and field_value:
        threshold_ms = float(field_value)
        threshold_kts = threshold_ms * 1.943844
        data_to_plot = loader.select_threshold(threshold_ms)
        # Look up the field name from config
        pow_fields = config.get_probability_fields(parameter)
        field_name = None
        for name, val in pow_fields.items():
            if abs(val - threshold_ms) < 0.1:
                field_name = name.split(" (")[0]  # Get "PoW11" from "PoW11 (11 kts)"
                break
        field_label = f" - {field_name}" if field_name else f" - PoW >{threshold_kts:.0f} kts"
        colorscale_type = "PoP"
