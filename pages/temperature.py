"""Temperature page for screen and pressure level temperature visualization."""
...
def update_parameter_options(step, cycle, realm):
    ...
    return create_dropdown_options(options, "tempscreen")
...
def update_field_options(output, validtime, parameter, step, cycle, realm):
    ...
    # Probability outputs - use POT fields from config
    if output in PROBABILITY_OUTPUTS:
        pot_fields = config.get_probability_fields(parameter)
        if pot_fields:
            options = [{"label": k, "value": str(v)} for k, v in pot_fields.items()]
            default_value = str(list(pot_fields.values())[len(pot_fields)//2])
...
def load_and_process_data(realm, cycle, step, parameter, output, validtime, field_value, use_native_proj=False):
    ...
    if data_type == "threshold" and field_value:
        ...
        pot_fields = config.get_probability_fields(parameter)
        field_name = None
        for name, val in pot_fields.items():
            if abs(val - threshold_k) < 0.1:
                field_name = name.split(" (")[0]
                break
        field_label = f" - {field_name}" if field_name else f" - POT >{threshold_c:.1f}C"
        colorscale_type = "PoP"
