"""Precipitation page for 24h precipitation visualization."""
...
# Register page
dash.register_page(__name__, path="/precipitation", name="Precipitation")

# Page-specific parameters
PAGE_PARAMS = ["precipacc24h", "precipacc03h"]
PAGE_ID = "precip"
...
layout = html.Div(
    [
        ...
        # Controls row 2: Field dropdown (for threshold/percentile selection)
        html.Div(
            [
                html.Div(
                    [
        ...
        # Controls row 3: colorscale, projection, and comparison toggles
        html.Div(
            [
...
def update_parameter_options(step, cycle, realm):
    ...
    return create_dropdown_options(options, "precipacc24h")
...
# Field dropdown visibility and options based on output type
@callback(
...
def update_field_options(output, validtime, parameter, step, cycle, realm):
    ...
    visible_style = {"width": "200px", "display": "inline-block", "marginRight": "10px"}

    if not output or not validtime:
        return hidden_style, [], None

    # Check if this output has threshold data (probabilities)
    if output in PROBABILITY_OUTPUTS:
        # Use predefined PoP fields from config
        pop_fields = config.get_probability_fields(parameter)
        if pop_fields:
            options = [{"label": k, "value": str(v)} for k, v in pop_fields.items()]
            return visible_style, options, str(list(pop_fields.values())[0])
        return hidden_style, [], None

    # Check if this output has percentile data
    if output in PERCENTILE_OUTPUTS:
        # Use percentile fields from config
    ...
    # No field selection needed for expected values
    return hidden_style, [], None
...
def load_and_process_data(realm, cycle, step, parameter, output, validtime, field_value, use_native_proj=False):
    ...
    Returns:
        ...
        x/y are either lon/lat (if use_native_proj=False) or native grid coords in km (if True)
        proj4str is the projection string (only meaningful when use_native_proj=True)
    """
    ...
    field_label = ""
    proj4str = loader.proj4str  # Get the projection string from the data
    ...
    if data_type == "threshold" and field_value:
        ...
        # Look up the field name from config
        pop_fields = config.get_probability_fields(parameter)
        field_name = None
        for name, val in pop_fields.items():
            if abs(val - threshold_m) < 0.0001:
                field_name = name.split(" (")[0]  # Get "PoP" from "PoP (0.2mm)"
                break
        field_label = f" - {field_name}" if field_name else f" - PoP >{threshold_mm:.1f}mm"
        colorscale_type = "PoP"
    ...
    if use_native_proj:
        # Return native grid coordinates (convert from m to km for readability)
        x_coords = data_to_plot.projection_x_coordinate.values / 1000  # m to km
        y_coords = data_to_plot.projection_y_coordinate.values / 1000  # m to km
...
def update_plot(validtime, field_value, colorscale_mode, projection_mode, compare_value, realm2,
    ...
    try:
        ...
        result1 = load_and_process_data(realm, cycle, step, parameter, output, validtime, field_value, use_native_proj)
        if result1 is None:
        ...
        if use_preset:
            ...
            # Get colorbar properties (must be after get_colorscale to populate cache)
        ...
        # Get coastlines - use the data's projection string for Albers mode
        if use_native_proj:
        ...
        if compare_mode:
            ...
            result2 = load_and_process_data(realm2, cycle, step, parameter, output, validtime, field_value, use_native_proj)
            ...
            if diff_max == 0:
                diff_max = 1  # Avoid zero range

            # Create 2 rows: maps on top, histograms on bottom
            # Use moderate horizontal_spacing for plot separation
            fig = make_subplots(
                rows=2, cols=3,
            ...
            # Colorbar settings
            # With 3 cols and 8% spacing, each plot is ~28% wide
            # Col 1: 0.00-0.28, Col 2: 0.36-0.64, Col 3: 0.72-1.00
            # Position colorbars just to the right of each plot
            cbar_len = 0.4
            cbar_y = 0.82
            cbar_thickness = 12
            cbar_base = {"len": cbar_len, "y": cbar_y, "thickness": cbar_thickness, "title": cbar_props.get("cbar_label", "")}

            # Add heatmap for realm 1 - colorbar at x=0.30
            fig.add_trace(
                go.Heatmap(
                    x=x1, y=y1, z=values1,
            ...
            # Add heatmap for realm 2 - colorbar at x=0.66
            fig.add_trace(
                go.Heatmap(
                    x=x2, y=y2, z=values2,
                    colorscale=colorscale, zmin=vmin, zmax=vmax,
            ...
            # Add difference heatmap - colorbar at x=1.02
            fig.add_trace(
                go.Heatmap(
            ...
            fig.update_layout(
                ...
                height=900,
                margin=dict(l=50, r=80, t=100, b=50),  # Extra right margin for colorbar
            )
            ...
            # Build colorbar settings - automatic display
